...
public class TransferDaoImpl implements TransferDao {
	...
	public TransactionResponseDto fundTranfer(TransactionRequestDto transactionrequest) throws InsufficientFundsException, ParseException{
		...
		if(accounts.get(0).getBalance()>0) {
			...
			if(transactionrequest.getTransactionTime()==null||transactionrequest.getTransactionTime().isEmpty()) {
		    transactionrequest.setTransactionTime(dtf.format(now));
			}else {
				SimpleDateFormat dateFormat=new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
				dateFormat.parse(transactionrequest.getTransactionTime());
	...
	public TransactionResponseDto getTransactionsByAccountId(int accountId) throws ExceptionById {
		...
		List<Transaction> transactions=getSession().createCriteria(Transaction.class).list();
		transactionResponseDto.setMessage(transactions.toString());
